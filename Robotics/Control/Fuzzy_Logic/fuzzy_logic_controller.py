...
pid= pidcont(.12,0.02,0.01,5,-5)
pid2= pidcont(.12,0.02,0.01,5,-5)
...
for i in range(len(d)):
    ...
    print(f"PID: KP: {newpid*pid.kp},  KI: {newpid*pid.ki},  KD: {newpid*pid.kd}")
    ...
    plt.grid()
    plt.plot(range(len(m)) ,m , 'r'  ,linewidth=1.5 , label='fuzzyPID')
    plt.plot(range(len(m2)),m2, 'b'  ,linewidth=1.5 , label='classicPID')
    plt.plot(range(len(d)) ,d , 'g--',linewidth=1, label='setpoint')
    # plt.legend()
    plt.title('Status')
